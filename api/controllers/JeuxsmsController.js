...
module.exports = {
  ...
  getHomeFront: async function (req, res) {
    console.log('getHomeFront gratuit')
    const datas = await Jeuxsms.find({ sort: 'createdAt DESC', where: { status: 'actif', homepage: true } })
    try {
  ...
  getAllFront: async function (req, res) {
    ...
    const datas = await Jeuxsms.find({ sort: 'createdAt DESC', where: { status: 'actif' } })
    try {
      res.status(200).json(datas)
    } catch (err) {
      res.status(500).json({ error: 'database error' })
  ...
  getOneFront: async function (req, res) {
    ...
    const datas = await Jeuxsms.findOne(req.params.id)
    try {
  ...
  getAll: async function (req, res) {
    console.log('GET ALL  jeuxgratuis')
    const datas = await Jeuxsms.find({ sort: 'createdAt DESC' })
    try {
      res.status(200).json(datas)
    } catch (err) {
      res.status(500).json({ error: 'database error' })
  ...
  add: async function (req, res) {
    ...
    item.codesms = req.body.data.codesms
    item.design = req.body.data.design
    item.status = req.body.data.status
    ...
    req.file('file').upload({
      ...
      try {
        const datas = await Jeuxsms.create(item).fetch()
        res.status(200).json(datas)
      } catch (error) {
        res.status(500).json({ message: error.message })
      }
  ...
  update: async function (req, res) {
    ...
    req.file('file').upload({
      ...
      const datas = await Jeuxsms.updateOne(req.params.id).set(item)
      try {
        console.log('datas')
        console.log(datas)
        res.status(200).json(datas)
  ...
  delete: async function (req, res) {
    ...
    const datas = await Jeuxsms.destroy(req.params.id)
